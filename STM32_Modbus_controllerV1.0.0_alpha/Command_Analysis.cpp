...
void Command_Analysis::Receive_Data_Analysis(void)
{
	...
	{
		/*通用指令*/
	case Work_Para: Query_Current_Work_Param();  break;//服务器查询当前群控相关参数，如所在区域、SN码、路数、工作组、等
	case Set_Group_Num: Set_Group_Number();         break;//设置本设备的工作组号，不需要验证本设备原有工作组号
	case SN_Area_Channel: Set_SN_Area_Channel();      break;//设置本设备的SN码、区域号、设备路数等参数
	case Work_Status: Detailed_Work_Status();     break;//查询本设备详细工作状态
	// case Stop_Work: Stop_Work_Command();        break;//强制停止当前设备的工作
	// 	/*卷膜机私有指令*/
	// case ResetRoll: ResetRoll_Command();        break;//重置卷膜测量行程
	// case Opening: Opening_Command();          break;//设置卷膜开度
	// case Work_Limit: Working_Limit_Command();    break;//电机工作电压阈值、上报状态间隔值设置
	}
}
