...
void Receipt::Working_Parameter_Receipt(bool use_random_wait, unsigned char times)
{
	...
	ReceiptFrame[ReceiptLength++] = SN.Read_RealTime_Opening_Value(); //电机实时开度
	unsigned int Temp = SN.Read_Roll_Low_Voltage_Limit_Value(); //读取最小电压阈值
	ReceiptFrame[ReceiptLength++] = highByte(Temp);
	ReceiptFrame[ReceiptLength++] = lowByte(Temp);
	Temp = SN.Read_Roll_High_Current_Limit_Value(); //读取最大电流阈值
	ReceiptFrame[ReceiptLength++] = highByte(Temp);
	ReceiptFrame[ReceiptLength++] = lowByte(Temp);
	Temp =0x00; //得到电机当前电流值
	ReceiptFrame[ReceiptLength++] = highByte(Temp);
	ReceiptFrame[ReceiptLength++] = lowByte(Temp);
	ReceiptFrame[ReceiptLength++] = Type_Conv.Dec_To_Hex(SN.Read_Roll_Report_Status_Interval_Value()); //得到上报状态频率
