...
class ModbusSerial : public Modbus {
    ...
    public:
        ...
        #endif
		//void task();
        void task(unsigned char* modbusPacket, int Length);
        bool receive(byte* frame);
        bool sendPDU(byte* pduframe);
        bool send(byte* frame);

		//自行添加的东西
		//unsigned char* Get_Modbus_Send_frame(unsigned char* frame);
		//unsigned char Get_Modbus_Send_len(unsigned char len);
};
